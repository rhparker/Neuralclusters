...
\end{equation}
As $N \rightarrow \infty$, $\beta_C^* \rightarrow 2 p-1$. We note that the largest valid value of $\beta_C$ is $\beta_C = n_C - 1$, which occurs when $n_{C_1} = n_C - 1$ and $n_{C_2} = 1$. For large $N$, since $p = f N/n_C$, the consistency condition $\beta < \beta_C^*$ is always satisfied, and thus \cref{eq:xEhat12IlargeN} holds, as long as $n_C < 2 p$. If $n_C$ is held constant, $\beta < \beta_C^*$ will be satisfied for sufficiently large $N$. 
...
\end{align}
for $N$ large. When $\beta_C = 1$, this reduces to $g_I(1) = \frac{\sqrt{N}}{\alpha \mu}$, which is \cref{eq:gpitchinhbeta1}. We note that for $N$ large, $g_I(\beta_C)$ is quadratic in $\beta_C$, has a local minimum at $\beta_C = 1$, and is increasing for $\beta_C > 1$.
