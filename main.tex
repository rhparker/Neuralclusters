...
\end{align*}
which is always negative. This eigenvalue will also never affect stability.

Finally, we use this method for $\lambda = \lambda_0 \pm \omega_0$. $H_3(\xvec)$ has a complex conjugate pair of eigenvalues, where the real part is given by
\begin{equation}
\lambda(g) = \frac{\mu_{EE}}{2}\left( \alpha - 1 + \alpha \beta g^2 (n_I-1) x_{I_1}^2 \right)
\end{equation}
...
\begin{equation*}
\lambda(g) = \frac{\mu_{EE} g}{2 \sqrt{N}}\left( \alpha - 1 + \alpha \beta (n_I-1) \frac{ 3(g - g_0) }{ (1 - \beta + \beta^2 )g} \right) - 1
\end{equation*}
to leading order. To locate the Hopf bifurcation, which occurs when the complex pair of eigenvalues crosses the imaginary axis, we solve $\lambda(g) = 0$, substitute $g_0 = \sqrt{N}/\alpha \mu_{EE}$, and simplify to get
