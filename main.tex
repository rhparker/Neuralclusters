...
When $\beta_C = 1$, $x_I = 0$, and this bifurcation takes place at $g = \frac{\sqrt{N}}{\alpha \mu}$. For $\beta_C > 1$, this takes place at $g$ much greater than $g_C$, thus the approximations \cref{eq:XE1E2asymp} and \cref{eq:XE1} do not hold.
...
\end{equation}
As $N \rightarrow \infty$, $\beta_C^* \rightarrow 2 p-1$. We note that the largest valid value of $\beta_C$ is $\beta_C = N_C - 1$, which occurs when $n_{C_1} = N_C - 1$ and $n_{C_2} = 1$. For large $N$, the consistency condition $\beta < \beta_C^*$ is always satisfied, and thus \cref{eq:xEhat12IlargeN} holds, as long as $n_C < 2 p$. If $n_C$ is held constant, $\beta < \beta_C^*$ will be satisfied for sufficiently large $N$. 
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
as $g \rightarrow \infty$. As $N \rightarrow \infty$ as well, the second term in the denominator dominates, thus \cref{eq:ye1hatyihat} simplifies to
\begin{equation}
        \hat{y}_{E_1} \rightarrow \frac{p}{1 + \beta_C - p}, \quad 
        \hat{y}_{I} \rightarrow \frac{p-1}{1 + \beta_C - p}.
\end{equation}
It follows that as $g, N \rightarrow \infty$,
\begin{equation}
    \begin{aligned}
        x_{E_1} &\rightarrow \frac{1}{g} \tanh^{-1} \left( \frac{p}{1 + \beta_C - p} \right) \\
        x_{E_2} &\rightarrow \frac{\mu}{\sqrt{N}}\left( -(p-1)n_C - \frac{p^2 n_C}{1 + \beta_C - p} \right) \\
        x_{I} &\rightarrow \frac{1}{g} \tanh^{-1} \left( \frac{p-1}{1 + \beta_C - p} \right).
    \end{aligned}
\end{equation}



We can use these formulas to find the location of the symmetric pitchfork bifurcations on the $C_1/C_2$ branches when $\beta_C>1$. At this bifurcation, the set of $n_{I} - 1$ eigenvalues of $D\tilde{F}(\xvec^*)$ located at $\frac{g}{\sqrt{N}}\alpha \mu \sech^2(g x_{I}) - 1$ cross through 0. Using the identity $\sech^2(g x_{I}) = 1 - \tanh^2(g x_{I}) = 1 - \hat{y}_I$ as $g \rightarrow \infty$ together with \cref{eq:yihat}, the symmetric pitchfork bifurcation is located at 
\begin{equation}\label{eq:gbetaC}
    g_I(\beta_C) = \frac{\sqrt{N}}{4 \alpha \mu} \frac{(1 + \beta_C)^2}{\beta_C} + \mathcal{O}\left(\frac{1}{N^{1/2}}\right)
\end{equation}
for $N$ large. We note that for $N$ large, $g_I(\beta_C)$ is quadratic in $\beta_C$, has a local minimum at $\beta_C = 1$, and is increasing for $\beta_C > 1$.
