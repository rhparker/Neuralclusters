...
For $\beta > 1$, it is no longer true that $x_{I_2} = -x_{I_1}$. However, by making an appropriate ansatz and proceding as described in \cref{app:I1I2sol}, 
...
\]
We analyze this by consideration the possible behavior of  $x_E$ as $g \rightarrow \infty$. 
\begin{itemize}
    \item If $x_E \rightarrow 0$, and $g x_E \rightarrow 0$ (e.g. if $x \approx g^{-\beta}$, for $\beta > 1$): $\sech^2(gx_E) \rightarrow 1$ and so $g \sech^2(gx_E) \rightarrow \infty$.
    \item If $x_E \rightarrow 0$, but $g x_E \rightarrow C$, for $C>0$ (e.g. $x \approx g^{-1}$): $g\sech^2(gx_E) \approx g\sech^2(C) \rightarrow \infty$.
     \item If $x_E \rightarrow 0$, but $g x_E \rightarrow \infty$ (e.g. if $x \approx g^{-\beta}$, for $0 < \beta < 1$): in this case, it is possible for $g\sech^2(gx_E) \rightarrow 0$; this would result in a stable fixed point. \textbf{However we have not observed this numerically} \textcolor{red}{True? False?}
\end{itemize} 
Thus for sufficiently large $g$, $\text{Trace }D\tilde{F}(\xvec^*) > 0$, which implies that $D\tilde{F}(\xvec^*)$ has an eigenvalue with positive real part. 

The remaining possibility is that $x_E \rightarrow \hat{x}_E \neq 0$. In Appendix \ref{app:stab_large} , we show that this cannot occur. Therefore, all $(x_E,x_{I_1},x_{I_2})$ satisfy $x_E \rightarrow 0$ as $g\rightarrow$ and are unstable for sufficiently large $g$.
...
\end{equation}
for $g$ close to $g_C$. Following the same procedure as in \S \ref{sec:E1I1}, we obtain the formula
...
As $g$ is further increased from $g_C$, there is a second symmetric pitchfork bifurcation on each $C_1/C_2$ branch as the eigenvalue $\lambda_I^*(g)$ of $D\tilde{F}(\xvec^*)$ crosses the origin (see \cref{fig:clusterBD1} and \cref{fig:clusterBD2}). The behavior at this symmetric pitchfork bifurcation is much the same as in \S \ref{sec:E1I1}: there is an $I_1/I_2$ branch of solutions for every possible division of the inhibitory cells into exactly two clusters. We can characterize these branches using the parameter $\beta = n_{I_1}/n_{I_2}$, as we did in the previous section. When $\beta_C = 1$, $x_I = 0$, and this bifurcation takes place at 
...
\section{Stability of the $I_1/I_2$ branch for large $g$: detailed calculations }\label{app:stab_largeg} 

Here we prove our assertion, made in \S \ref{sec:stab_largeg}, that $x_E \rightarrow 0$ as $g\rightarrow \infty$ along any $I_1/I_2$ solution branch.
...
\]
where we used the fact that $n_C p = n_E = \alpha n_I$. Since we are taking $\beta \geq 1$, this simplifies to the equation given in \eqref{eq:betacond}.
...
\]
which simplifies to Eq. \eqref{eq:bccondition}
