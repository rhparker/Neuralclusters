...
\noteAKB{Need to check what follows. Symmetry group is different because it includes now $S_{n_{C_1}}\times S_{n_{C_2}}$.  Also check that $V$ is as in 6.1; probably true but double-check.}
\revised{
The behavior at this bifurcation is exactly the same as for the second symmetric pitchfork bifurcation at the origin, since the symmetry group and fixed-point subspace are identical to that case (see \cref{sec:Eclusterbiforigin}). It follows from the Equivariant Branching Lemma that, on every $C_1/C_2$ branch, there is an $I_1/I_2$ branch of solutions for every possible division of the inhibitory cells into exactly two clusters. 
}
