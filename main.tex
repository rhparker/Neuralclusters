...
\]
We analyze this by considering the behavior of $x_E$ as $g \rightarrow \infty$. First, we consider the case when $x_E \rightarrow 0$. There are three possibilities, only two of which can occur.
\begin{itemize}
    ...
    \item $x_E \rightarrow 0$, but $g x_E \rightarrow \infty$ (e.g. $x \sim g^{-\beta}$, for $0 < \beta < 1$). We will show this cannot happen. If this is true, then $g\sech^2( g x_E) \rightarrow 0$, from which it follows that $\tanh^2( g x_E) \rightarrow 1$, which implies $\tanh(g x_E) \rightarrow 1$. Since $x_{I_1}$ and $x_{I_2}$ have opposite signs for $g > g_0$ (see the end of \cref{app:I1I2sol}), it follows from \cref{eq:reducedsystemI1I2} that
    \[
    x_E \geq \frac{\mu_{EE}}{\sqrt{N}}\left[ (\alpha n_I - 1) - \alpha n_I \frac{\beta}{\beta+1} \right] = \frac{\mu_{EE}}{\sqrt{N}}\left[ \alpha(n_I - n_{I_1}) - 1 \right] \geq \frac{\mu_{EE}}{\sqrt{N}}\left( \alpha - 1 \right),
    \]
    since $n_{I_1} \leq n_I - 1$. At long we take $\alpha > 1$ (which is typically the case), $x_E$ is bounded away from 0 for all $g > g_0$, thus we cannot have $x_E \rightarrow 0$.
\end{itemize} 
...
\end{aligned}
\end{equation}
for $g$ close to $g_C$. The cluster-associated eigenvalue $\lambda_{C_1}^*(g)$ is negative for $1 \leq \beta_C < 2$ and positive for $\beta_C > 2$; $\lambda_{C_2}^*(g)$ is negative for $\beta_C  > 1/2$; and $\lambda_{I}^*(g)$ is negative for all $\beta_C$ for $N$ sufficiently large. These imply that the $C_1/C_2$ branch is initially unstable for $\beta_C > 2$ (see \cref{fig:clusterBD1} and \cref{fig:clusterBD2}). 
...
\end{equation}
As $N \rightarrow \infty$, $n_C p = f N \rightarrow \infty$ as well. If both $p$ and $n_C$ scale as $\sqrt{N}$, then the only significant terms in the numerator and denominator of \cref{eq:betaCstar} are of order $N$ or larger, in which case $\beta_C^* \rightarrow 2 p-1$ as $N \rightarrow \infty$. \textbf{Next statement seems to contradict what I just read. I took that out part out. Maybe just include what I have above?} 
% This depends on neither $p$ nor $n_C$, since these variables are involved in \cref{eq:betaCineq} only in the form $p n_C = n_E = f N$. We note that the largest possible value of $\beta_C$ is $\beta_C = n_C - 1$, which occurs when $n_{C_1} = n_C - 1$ and $n_{C_2} = 1$. Since $p = f N/n_C$, it follows that if either $n_C$ is held constant or $n_C$ scales as $\sqrt{N}$, $\beta_C < \beta_C^*$ will be always satisfied for sufficiently large $N$.
...
\subsection{Stable fixed points for large \texorpdfstring{$g$}{g}}  \label{sec:C1C2I1I2_largeg}
