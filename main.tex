...
Main points to make
\begin{itemize}
    \item Biological neural networks show great diversity in connectivity structure. How does connectivity determine the dynamics? Which features are determinative vs. incidental.
    \item Because neural networks are large, under-specified and heterogeneous, random matrix theory has played a prominent role. Summarize Sompolinsky study here.
    \item Numerous authors have considered modifications such as low-rank or deterministic perturbations and block-structured matrices, and have found how Sompolinsky's conclusions must be modified accordingly.
    \begin{itemize}
        \item However, all of these works focus on phase space structures that occur as a result of randomness.
        \item Another possible feature is phase space structures that exist \emph{in spite of} randomness, and that might be found in some idealized (averaged?) version of the network. These structures might not be predicted by random matrix theory, as the spectrum of the connectivity matrix will ``swamp" the spectrum of the averaged version.
    \end{itemize}
    \item In an earlier work, we found just such an example (explain more). However, some pieces of our analysis remained uncompleted. In particular, we were unable at that time to give a complete explanation of when the fixed points/limit cycles would be stable (i.e. for which $g$). This is important because the stable solution is what one can expect to observe in a perturbed (random) network. 
    \item Here, we complete this analysis for all-to-all excitatory-inhibitory networks. We then extend this analysis to a biologically significant block-structured case: in which the excitatory cells are clustered but inhibition is global. We find that the dynamics are strikingly different: instead of limit cycles, we predict fixed points. In both cases, the structures can be understood by considering the symmetries of the deterministic connection matrix.  
...
Features of the system such as fixed points and periodic orbits remain when the connectivity matrix is perturbed by a random matrix, i.e. $G=H + \epsilon A$ for small $\epsilon$ [REFERENCE], even when the spectrum of $G$ bears no obvious resemblance to the original spectrum of $H$. We first reported on this phenomenon in \cite{Barreiro2017}, where we analyzed all-to-all connected, balanced excitatory-inhibitory networks ($n_C = 1$ and $n_{C_I} = 1$). In this paper, we first flesh out some details about that system; in particular, we derive leading order expressions for bifurcation points in the system, as well as for the equilibria near those bifurcation points. We then extend the analysis to several families of networks, in which the cell populations are split up into clusters of equal sizes. For the remainder of this paper we will use $f = 0.8$ for a 4-to-1 excitatory-to-inhibitory ratio, which is typical for cortical networks [REFERENCE].
...
\begin{figure}
    ...
    \caption{Semi-log plot of log of absolute error of approximation \cref{eq:ghopfformula} vs $N$ for $\beta = 1$, 3, and 4 (left). Slope of line is approximately -1.5 for all three $\beta$. Right is location in $(N, g)$ space of symmetric pitchfork bifurcation at $g_0$ (dashed line), Hopf bifurcation at origin at $g_H$ (dash-dotted line), and Hopf bifurcations on $I_1/I_2$ branches for select $\beta$ (solid lines, arranged from bottom to top in increasing $\beta$) on semi-log plot. $\alpha = 4$, $\mu_{EE}= 0.7$. }
    \label{fig:Hopfplots}
\end{figure}
...
\begin{figure}
    \centering
    % \begin{tabular}{cc}
    \includegraphics[width=7.9cm]{images/periodvsg.eps}
    \includegraphics[width=7.9cm]{images/floquet1.eps}
...
\begin{figure}
    \centering
    % \begin{tabular}{cc}
    \includegraphics[width=7.5cm]{images/bdclusters20c4E.eps}\hspace{-0.5cm}
    ...
    % \end{tabular}
    \caption{Left panel shows excitatory cell activity $x_{E_1}$ and $x_{E_2}$ on $C_1/C_2$ branches of equilibria of \cref{eqn:sys_Basic} with excitatory clustering for all valid values of $\beta_C$, i.e. $\beta_C =1$ and $\beta_C=3$. Symmetric pitchfork bifurcations at $g = g_C$ and along the $C_1/C_2$ branches indicated with filled circle. (No $I_1/I_2$ branches after symmetric pitchfork bifurcation on $C_1/C_2$ branch are shown to avoid clutter). Right panel shows $I_1/I_2$ branches bifurcating from $C_1/C_2$ branches (only $x_{E_1}$ is shown to avoid clutter). Stable fixed points are indicated with solid lines. $N = 20$, $n_C = 4$, $p = 4$, $n_I = 4$, $\alpha = 4$, $\mu_{EE} = 0.7$.}
...
\begin{figure}
    ...
    % \end{tabular}
    \caption{Eigenvalue pattern of connectivity matrix $H(\xvec^*)$ for fixed point $\xvec^*$ on $C_1/C_2$ branch with $\beta_C > 1$. The notation for the eigenvalues is explained below. \textbf{Is $\lambda_C$ an eigenvalue that has split into $\lambda_{C_1}$ and $\lambda_{C_1}$? Let's be precise. Yes, explained briefly below.}}
