...
For $\beta_C > 1$, numerical parameter continuation with AUTO suggests that while 
$x_I \rightarrow 0$ as $g \rightarrow \infty$, $\tanh(g x_I) \rightarrow \hat{y}_I \neq 0$.
There are two possibilities for the limiting behavior of $x_{E_1}$ and $x_{E_2}$. If $1 < \beta_C < \beta^*$, where $\beta^*$ will be determined below, $x_{E_1} \rightarrow \hat{x}_{E_1} > 0$ and $x_{E_2} \rightarrow \hat{x}_{E_2} < 0$ as $g \rightarrow \infty$, which implies that $\tanh(g x_{E_1}) \rightarrow 1$ and $\tanh(g x_{E_2}) \rightarrow -1$. Using this ansatz, we can we can solve for $\hat{y}_I$ using row 3 of \cref{eq:cluster3system} to get
