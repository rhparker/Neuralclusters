...
First, we look at the behavior of the $C_1/C_2$ branch as $g$ becomes large. Numerical parameter continuation with AUTO suggests that as $g \rightarrow \infty$, $x_{E_1} \rightarrow \hat{x}_{E_1} > 0$ and $x_{E_2} \rightarrow \hat{x}_{E_2} > 0$. This implies that $\tanh(g x_{E_1}) 1$ and $\tanh(g x_{E_2}) \rightarrow -1$ as $g \rightarrow \infty$. When $\beta_C = 1$, $x_I = 0$ for all $g$, thus it follows from \cref{eq:cluster3system} that $\hat{x}_{E_1} = \frac{\mu}{\sqrt{N}}(p-1)n_C$ and $\hat{x}_{E_2} = -\frac{\mu}{\sqrt{N}}(p-1)n_C$. When $\beta_C > 1$, numerical parameter continuation suggests that while $x_I \rightarrow 0$ as $g \rightarrow \infty$, $\tanh(g x_I) \rightarrow \hat{y}_I \neq 0$. Using row 3 of \cref{eq:cluster3system}, we can solve for $\hat{y}_I$ to get
...
\end{aligned}
\end{equation}
Since $n_C p = f N$, $\hat{y}_I \rightarrow \frac{\beta_C - 1}{\beta_C+1}$ as $N \rightarrow \infty$. Thus as $g, N \rightarrow \infty$, the solution on the $C_1/C_2$ branch for $\beta_C > 1$ is given by 
\begin{equation}
    \begin{aligned}
        x_{E_1} &\rightarrow\frac{\mu}{\sqrt{N}}\left( (p-1)n_C - \frac{\beta_C - 1}{\beta_C+1} \right) \\
        x_{E_2} &\rightarrow \frac{\mu}{\sqrt{N}}\left( -(p-1)n_C - \frac{\beta_C - 1}{\beta_C+1} \right) \\
