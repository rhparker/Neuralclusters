...
Choose any $\beta_C \geq 1$, so that $n_{E_1} = \frac{\beta_C}{\beta_C+1}n_I$ and $n_{E_2} = \frac{1}{\beta_C+1}n_I$. For stability of solutions on the $C_1/C_2$ branches, we follow the same procedure as with the $I_1/I_2$ branches in the previous section.  We linearize the reduced system \cref{eq:cluster3system} about the branch $(x_{E_1}, x_{E_2}, x_{I})$ to get the Jacobian
\[
J_3(\xvec) = \frac{g}{\sqrt{N}} H_3(\xvec) - I_3
\]
where 
...
\begin{bmatrix} 
 ...
 \end{bmatrix}
\]
and $I_3$ is the $3 \times 3$ identity matrix. 
